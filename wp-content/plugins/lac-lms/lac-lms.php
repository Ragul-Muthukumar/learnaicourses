 ...
 * 3) Require common, db, validation, CPT, PayPal, enrollment, REST, and seed modules.
 * 4) Hook activation to create tables and seed demo courses once.
 * 5) Upgrade schema on plugins_loaded for existing installs.
 */
...
require_once LAC_LMS_PATH . 'includes/enrollment.php';
 ...
 // Theme-independent course product templates.
require_once LAC_LMS_PATH . 'includes/templates.php';
...
require_once LAC_LMS_PATH . 'includes/seed-course-images.php';
 ...
 // Default curriculum seeder for courses without lessons.
require_once LAC_LMS_PATH . 'includes/seed-course-lessons.php';
...
function lac_lms_activate() {
	...
	update_option( 'lac_lms_db_version', '1.1.0' );
...
function lac_lms_maybe_upgrade_schema() {
	...
	lac_log_info( 'LAC LMS schema upgraded to 1.1.0.' );
}
...
function lac_lms_enqueue_assets() {
	...
	if ( is_post_type_archive( 'lac_course' ) || is_singular( array( 'lac_course', 'lac_lesson' ) ) || is_front_page() ) {
		...
		wp_enqueue_style(
		 ...
		 // Load the PayPal JS SDK only when credentials exist.
		if ( lac_paypal_is_configured() ) {
		 ...
		 // Register the shared front-end script that posts enrollments / captures.
		wp_enqueue_script(
			'lac-lms-common',
			LAC_LMS_URL . 'assets/js/common.js',
			lac_paypal_is_configured() ? array( 'lac-paypal-sdk' ) : array(),
			LAC_LMS_VERSION,
			true
		);

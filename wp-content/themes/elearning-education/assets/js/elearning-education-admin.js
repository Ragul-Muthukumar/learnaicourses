    ...
    if (closeBtn && detailBox) {
        closeBtn.addEventListener("click", function(e) {
            ...
            fetch(ajaxurl, {
                ...
                body: 'action=elearning_education_dismissed_get_started_detail_notice'
            })
...
function elearning_education_open_tab(evt, tabName) {
    let i, tabcontent, tablinks;

    // Hide all tabs
    tabcontent = document.getElementsByClassName("elearning-education-tabcontent");
    for (i = 0; i < tabcontent.length; i++) {
        tabcontent[i].style.display = "none";
    }

    // Remove active from all buttons
    tablinks = document.getElementsByClassName("elearning-education-tablinks");
...
jQuery(document).ready(function ($) {
    // Hide all tabs first
    $(".elearning-education-tabcontent").hide();
    
    // **DIRECTLY** activate first tab (no trigger)
    let firstTab = $(".elearning-education-tablinks").first();
